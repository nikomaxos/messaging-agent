  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">Platform Testing</h1>
          <p className="text-slate-400 text-sm">Send live tests, emulate routing mechanisms, and stress test the platform.</p>
        </div>
      </div>

      <div className="flex gap-4 border-b border-white/10 mb-6 pb-2">
        <button onClick={() => setActiveTab('send')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${activeTab === 'send' ? 'bg-brand-600/20 text-brand-400 border border-brand-500/20' : 'text-slate-400 hover:text-white'}`}>
          <Send size={16} /> Send Message
        </button>
        <button onClick={() => setActiveTab('emulator')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${activeTab === 'emulator' ? 'bg-brand-600/20 text-brand-400 border border-brand-500/20' : 'text-slate-400 hover:text-white'}`}>
          <Terminal size={16} /> Emulator
        </button>
        <button onClick={() => setActiveTab('stress')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition ${activeTab === 'stress' ? 'bg-brand-600/20 text-brand-400 border border-brand-500/20' : 'text-slate-400 hover:text-white'}`}>
          <Zap size={16} /> Stress Testing
        </button>
      </div>

      {activeTab === 'send' && (
        <div className="bg-[#1a1a2e] border border-white/10 rounded-xl p-6 max-w-2xl">
          <h2 className="text-lg font-semibold text-white mb-4">Manual Single Message Test</h2>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-300 mb-1">Protocol</label>
                <select value={sendForm.protocol} onChange={e => setSendForm({...sendForm, protocol: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
              ...
              <div>
                ...
                <input type="text" value={sendForm.senderId} onChange={e => setSendForm({...sendForm, senderId: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-300 mb-1">Data Coding</label>
                <select value={sendForm.dataCoding} onChange={e => setSendForm({...sendForm, dataCoding: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
              ...
              <div className="flex items-end pb-2">
                <label className="flex items-center gap-2 cursor-pointer text-slate-300 text-sm">
                  <input type="checkbox" checked={sendForm.isFlash} onChange={e => setSendForm({...sendForm, isFlash: e.target.checked})} className="rounded bg-[#12121f] border-white/10 text-brand-500 focus:ring-brand-500/50 w-4 h-4" />
                  Flash SMS
                </label>
              </div>
            </div>
            <div>
              ...
              <input type="text" value={sendForm.destination} onChange={e => setSendForm({...sendForm, destination: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white" />
            </div>
            <div>
              <label className="block text-sm text-slate-300 mb-1">Force Supplier Connection (Optional)</label>
              <select value={sendForm.supplierId} onChange={e => setSendForm({...sendForm, supplierId: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
            ...
            <div>
              ...
              <textarea rows={4} value={sendForm.message} onChange={e => setSendForm({...sendForm, message: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white"></textarea>
            ...
            <button onClick={() => sendMut.mutate(sendForm)} disabled={sendMut.isPending} className="bg-brand-600 hover:bg-brand-500 text-white px-6 py-2 rounded-lg font-medium">
      ...
      {activeTab === 'emulator' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[700px]">
          <div className="bg-[#1a1a2e] border border-white/10 rounded-xl p-6 overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-slate-300 mb-1">Protocol</label>
                  <select value={emuForm.protocol} onChange={e => setEmuForm({...emuForm, protocol: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
                    <option value="SMS">Standard SMS</option>
                    <option value="RCS">RCS (via Mautrix)</option>
                    <option value="WHATSAPP">WhatsApp (via Mautrix)</option>
                    <option value="WEBSOCKET">WebSocket App</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-300 mb-1">Sender ID</label>
                  <input type="text" value={emuForm.senderId} onChange={e => setEmuForm({...emuForm, senderId: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white" />
                </div>
                <div>
                  <label className="block text-sm text-slate-300 mb-1">Destination Number</label>
                  <input type="text" value={emuForm.destination} onChange={e => setEmuForm({...emuForm, destination: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1">Data Coding</label>
                <select value={emuForm.dataCoding} onChange={e => setEmuForm({...emuForm, dataCoding: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
                  <option value="0">0 - Default (7-bit)</option>
                  <option value="3">3 - Latin-1 (8-bit)</option>
                  <option value="4">4 - Binary (8-bit)</option>
                  <option value="8">8 - UCS2 (Unicode)</option>
              ...
              <div>
                ...
                <select value={emuForm.supplierId} onChange={e => setEmuForm({...emuForm, supplierId: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
                  <option value="">-- Let Routing Engine Decide --</option>
                  {smscs.map((s: any) => <option key={s.supplier.id} value={s.supplier.id}>{s.supplier.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1">Message Body</label>
                <textarea rows={4} value={emuForm.message} onChange={e => setEmuForm({...emuForm, message: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white"></textarea>
                <div className="text-xs text-slate-500 mt-1 flex justify-end">
              ...
              <button onClick={() => emuMut.mutate(emuForm)} disabled={emuMut.isPending} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-lg font-medium">
                {emuMut.isPending ? 'Emulating...' : 'Emulate Routing'}
              </button>
            </div>
          </div>
          
          <div className="bg-[#0a0a0f] border border-white/10 rounded-xl overflow-hidden flex flex-col shadow-2xl">
              ...
              {!emuResult ? (
                ...
                <>
                  ...
                  <div className="mt-6 border-t border-white/10 pt-4">
                    ...
                    <pre className="text-xs bg-black/40 p-3 rounded overflow-x-auto text-slate-300 border border-white/5">
                      {JSON.stringify({
      ...
      {activeTab === 'stress' && (
        <div className="bg-[#1a1a2e] border border-white/10 rounded-xl p-6 max-w-2xl">
          <h2 className="text-lg font-semibold text-white mb-4">Volume Stress Test</h2>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-300 mb-1">Client Emulation</label>
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1">Protocol</label>
                <select value={stressForm.protocol} onChange={e => setStressForm({...stressForm, protocol: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
                  <option value="SMS">Standard SMS</option>
                  <option value="RCS">RCS (via Mautrix)</option>
                  <option value="WHATSAPP">WhatsApp (via Mautrix)</option>
                  <option value="WEBSOCKET">WebSocket App</option>
            ...
            <div>
              <label className="block text-sm text-slate-300 mb-1">Data Coding</label>
              <select value={stressForm.dataCoding} onChange={e => setStressForm({...stressForm, dataCoding: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
                <option value="0">0 - Default (7-bit)</option>
                <option value="3">3 - Latin-1 (8-bit)</option>
                <option value="4">4 - Binary (8-bit)</option>
                <option value="8">8 - UCS2 (Unicode)</option>
            ...
            <div>
              <label className="block text-sm text-slate-300 mb-1">Message Body</label>
              <textarea rows={2} value={stressForm.message} onChange={e => setStressForm({...stressForm, message: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white"></textarea>
              <div className="text-xs text-slate-500 mt-1 flex justify-end">
            ...
            <div className="bg-black/20 p-4 border border-white/5 rounded-lg space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                <div>
                  ...
                  <select value={stressForm.simulationMode} onChange={e => setStressForm({...stressForm, simulationMode: e.target.value})} className="w-full bg-[#12121f] border border-brand-500/30 rounded px-3 py-2 text-white font-medium">
                ...
                {stressForm.simulationMode === 'SIMULATE_DELIVERY' && (
                  <div>
                    ...
                    <select value={stressForm.forcedErrorCode} onChange={e => setStressForm({...stressForm, forcedErrorCode: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white">
