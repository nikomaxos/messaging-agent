  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {rules.map(r => (
          <div key={r.id} className={`bg-white dark:bg-slate-800 rounded-xl shadow-sm border p-4 flex items-center justify-between transition-colors hover:border-blue-400 ${!r.active ? 'opacity-75 border-slate-200 dark:border-slate-700' : 'border-slate-200 dark:border-slate-700'}`}>
        ...
        {rules.length === 0 && (
          <div className="text-center py-12 text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-600">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-700">
              {['BUILDER', 'TESTER', 'FLOWCHART'].map(tab => (
                <button
                  ...
                  className={`px-6 py-3 text-sm font-medium transition-colors ${activeTab === tab ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                >
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="p-6 space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-2" placeholder="e.g. Block Spam Links" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Description</label>
                      <input type="text" value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-2" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Priority (Lower runs first)</label>
                      <input type="number" value={priority} onChange={e => setPriority(parseInt(e.target.value))} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-2" />
                    </div>
                    ...
                    <div className="space-y-3">
                      {conditions.map((c, i) => (
                        <div key={i} className="flex gap-2 items-center bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                          ...
                          }} className="p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700">
                          ...
                          <select value={c.operator} onChange={e => {
                            ...
                            setConditions(newCond);
                          }} className="p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 text-blue-600 font-medium">
                          ...
                          <input type="text" value={c.value} onChange={e => {
                            ...
                            setConditions(newCond);
                          }} className="flex-1 p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 font-mono text-sm" placeholder="Value or Regex" />
                    ...
                    <div className="mt-4 pt-4 border-t border-orange-200 dark:border-orange-900/50">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {REGEX_SNIPPETS.map((snip, i) => (
                          <button key={i} title={snip.desc} onClick={() => setConditions([...conditions, { field: 'MESSAGE_TEXT', operator: 'MATCHES_REGEX', value: snip.regex }])} className="text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center gap-1 transition-colors">
                      ...
                      {actions.map((a, i) => (
                        <div key={i} className="flex gap-2 items-center bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                          <select value={a.actionType} onChange={e => {
                            ...
                            setActions(newActs);
                          }} className="p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 font-medium text-emerald-600">
                          ...
                          {a.actionType === 'FAKE_DLR' ? (
                            <select value={a.actionValue} onChange={e => {
                              ...
                              setActions(newActs);
                            }} className="flex-1 p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 font-mono text-sm">
                            ...
                            <input type="text" value={a.actionValue} onChange={e => {
                              ...
                              newActs[i].actionValue = e.target.value;
                              setActions(newActs);
                            }} className="flex-1 p-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 font-mono text-sm" placeholder={a.actionType.startsWith('REWRITE') ? 'search_regex|||replacement OR replacement' : 'Value (e.g. SMSC ID)'} />
                          )}
                ...
                <div className="p-6 flex flex-col lg:flex-row gap-6 h-full">
                  <div className="flex-1 space-y-4">
                    ...
                    <div>
                      ...
                      <input type="text" value={testPayload.systemId} onChange={e => setTestPayload({...testPayload, systemId: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border rounded-lg p-2" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Source Address</label>
                      <input type="text" value={testPayload.sourceAddress} onChange={e => setTestPayload({...testPayload, sourceAddress: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border rounded-lg p-2" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Destination Address</label>
                      <input type="text" value={testPayload.destinationAddress} onChange={e => setTestPayload({...testPayload, destinationAddress: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border rounded-lg p-2" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Message Text</label>
                      <textarea value={testPayload.messageText} onChange={e => setTestPayload({...testPayload, messageText: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border rounded-lg p-2 h-24" />
                    </div>

                    <button onClick={handleTest} className="w-full bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 py-3 rounded-xl font-bold flex justify-center items-center gap-2 hover:bg-slate-700 transition-colors">
                  ...
                  <div className="flex-1 bg-slate-900 rounded-xl p-4 text-emerald-400 font-mono text-sm overflow-y-auto">
                    <h4 className="text-white mb-2 pb-2 border-b border-slate-700">Execution Result:</h4>
                    {testResult ? (
              ...
              {activeTab === 'FLOWCHART' && (
                <div className="w-full h-full p-4 bg-slate-50 dark:bg-slate-900">
                  <div className="w-full h-full border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-800">
                    <ReactFlow 
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3 bg-white dark:bg-slate-800">
              ...
              <button onClick={handleSave} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg flex items-center gap-2 shadow-lg shadow-blue-500/30 transition-all">
