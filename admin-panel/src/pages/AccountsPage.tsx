    ...
    <div className="p-8 h-[calc(100vh-64px)] flex flex-col relative overflow-hidden">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          ...
          <p className="text-slate-400 text-sm">Manage billing accounts, customer details, and routing channels</p>
        </div>
        <button
          onClick={() => { setEditingAccountParams({}); setShowAccountModal(true); }}
          className="bg-brand-600 hover:bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2"
        >
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        {/* Left Sidebar (Master) */}
        <div className="w-1/3 max-w-sm bg-[#1a1a2e] border border-white/[0.05] rounded-xl flex flex-col overflow-hidden shadow-sm">
          <div className="p-4 border-b border-white/[0.05] bg-[#12121f]">
             <div className="relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
                <input 
                   type="text" 
                   placeholder="Search accounts..." 
                   className="w-full bg-[#1a1a2e] border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-brand-500 transition"
                   value={searchQuery}
        ...
        <div className="flex-1 flex flex-col gap-6 min-h-0">
           {selectedAccount ? (
              <>
                 ...
                 <div className="bg-[#1a1a2e] border border-white/[0.05] rounded-xl flex flex-col shrink-0 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.05] bg-[#12121f] flex justify-between items-center">
                       <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                          <Building2 size={18} className="text-brand-400" />
                          Account Information
                       </h2>
                       <div className="flex items-center gap-2">
                          <button onClick={() => { setEditingAccountParams(selectedAccount); setShowAccountModal(true); }} className="p-1.5 text-slate-400 hover:text-white hover:bg-white/10 rounded transition" title="Edit Account">
                             <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDeleteAccount(selectedAccount.id)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded transition" title="Delete Account">
                             <Trash2 size={16} />
                          </button>
                       </div>
                    </div>
                    <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-6">
                       <div>
                          ...
                          <p className="text-sm text-white font-medium">{selectedAccount.companyName || '-'}</p>
                       </div>
                       <div>
                 ...
                 <div className="bg-[#1a1a2e] border border-white/[0.05] rounded-xl flex flex-col flex-1 min-h-0 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.05] bg-[#12121f] flex justify-between items-center shrink-0">
                       <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                    ...
                    <div className="overflow-y-auto flex-1 p-0">
                       <table className="w-full text-left text-sm text-slate-300">
                         <thead className="bg-[#12121f]/50 text-slate-400 border-b border-white/[0.05] sticky top-0">
                           <tr>
                             ...
                             <tr key={idx} className="hover:bg-white/[0.02] transition">
                               ...
                               <td className="px-6 py-4">
                                  <div className="flex flex-col gap-1">
                                    <span className="text-xs text-slate-400">IP Lock: {u.enforceIpWhitelist ? <span className="text-green-400">ON</span> : 'OFF'}</span>
                               ...
                               <td className="px-6 py-4 text-right">
                                  <button onClick={() => { setEditingUsername(u); setEditingUsernameIndex(idx); setShowUsernameModal(true); }} className="p-1.5 text-slate-400 hover:text-white hover:bg-white/10 rounded transition mr-1" title="Edit Username"><Edit2 size={15} /></button>
                                  <button onClick={() => handleDeleteUsername(idx)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded transition" title="Delete Username"><Trash2 size={15} /></button>
                               </td>
                             </tr>
                           ))}
              ...
              <div className="flex-1 flex items-center justify-center bg-[#1a1a2e] border border-white/[0.05] rounded-xl">
      ...
      {showAccountModal && editingAccountParams && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1a1a2e] border border-white/10 rounded-xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" name="name" defaultValue={editingAccountParams.name} required className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Type *</label>
                    <select name="type" defaultValue={editingAccountParams.type || 'CUSTOMER'} required className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <input type="text" name="companyName" defaultValue={editingAccountParams.companyName} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">VAT Number</label>
                    <input type="text" name="vatNumber" defaultValue={editingAccountParams.vatNumber} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-xs font-medium text-slate-400 mb-1">Address</label>
                    <input type="text" name="address" defaultValue={editingAccountParams.address} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Email (Billing)</label>
                    <input type="email" name="email" defaultValue={editingAccountParams.email} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Contact Person</label>
                    <input type="text" name="contactPerson" defaultValue={editingAccountParams.contactPerson} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm" />
                  </div>
                </div>
              </div>
              <div className="px-6 py-4 border-t border-white/5 bg-[#12121f] flex justify-end gap-3 shrink-0">
                <button type="button" onClick={() => setShowAccountModal(false)} className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white transition">Cancel</button>
                <button type="submit" disabled={isSaving} className="bg-brand-600 hover:bg-brand-500 text-white px-6 py-2 rounded-lg text-sm font-medium transition disabled:opacity-50">Save</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showUsernameModal && editingUsername && (
        <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1a1a2e] border border-white/10 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-white/5 bg-[#12121f]">
              <h3 className="text-lg font-semibold text-white">
            ...
            <form onSubmit={handleSaveUsername} className="flex flex-col">
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <input type="text" value={editingUsername.username || ''} onChange={e => setEditingUsername({...editingUsername, username: e.target.value})} disabled={editingUsernameIndex !== null} required className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm disabled:opacity-50 disabled:cursor-not-allowed" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Whitelisted IPs (Comma separated)</label>
                  <input type="text" value={editingUsername.whitelistedIps || ''} onChange={e => setEditingUsername({...editingUsername, whitelistedIps: e.target.value})} className="w-full bg-[#12121f] border border-white/10 rounded px-3 py-2 text-white text-sm font-mono" />
                </div>
                <div className="pt-2 border-t border-white/5">
                   <h4 className="text-xs font-medium text-slate-400 mb-3">Capabilities</h4>
                   <div className="grid grid-cols-2 gap-4">
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-white/5 hover:bg-white/5 transition">
                        <input type="checkbox" checked={editingUsername.enforceIpWhitelist || false} onChange={e => setEditingUsername({...editingUsername, enforceIpWhitelist: e.target.checked})} className="form-checkbox text-brand-500 rounded bg-[#12121f] border-white/20" />
                      ...
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-brand-500/30 bg-brand-500/5 hover:bg-brand-500/10 transition">
                        <input type="checkbox" checked={editingUsername.smppEnabled || false} onChange={e => setEditingUsername({...editingUsername, smppEnabled: e.target.checked})} className="form-checkbox text-brand-500 rounded bg-[#12121f] border-white/20" />
                        <span className="text-sm text-brand-100 font-medium">SMPP Access</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-white/5 hover:bg-white/5 transition">
                        <input type="checkbox" checked={editingUsername.apiEnabled || false} onChange={e => setEditingUsername({...editingUsername, apiEnabled: e.target.checked})} className="form-checkbox text-brand-500 rounded bg-[#12121f] border-white/20" />
                        <span className="text-sm text-slate-300">HTTP API Access</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-white/5 hover:bg-white/5 transition">
                        <input type="checkbox" checked={editingUsername.webEnabled || false} onChange={e => setEditingUsername({...editingUsername, webEnabled: e.target.checked})} className="form-checkbox text-brand-500 rounded bg-[#12121f] border-white/20" />
                      ...
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-red-500/30 bg-red-500/5 hover:bg-red-500/10 transition col-span-2 mt-2">
                        <input type="checkbox" checked={editingUsername.banned || false} onChange={e => setEditingUsername({...editingUsername, banned: e.target.checked})} className="form-checkbox text-red-500 rounded bg-[#12121f] border-red-500/50" />
                        <span className="text-sm text-red-400 font-bold uppercase tracking-wider">BAN USER (Cut off all access)</span>
                      </label>
                   </div>
                </div>
              </div>
              <div className="px-6 py-4 border-t border-white/5 bg-[#12121f] flex justify-between items-center shrink-0">
                <div>
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setShowUsernameModal(false)} className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white transition">Cancel</button>
                  <button type="submit" disabled={isSaving} className="bg-brand-600 hover:bg-brand-500 text-white px-6 py-2 rounded-lg text-sm font-medium transition disabled:opacity-50">Save Username</button>
                </div>
              </div>
            </form>
          </div>
      ...
      {showSmppModal && pendingSmppConfigs.length > 0 && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1a1a2e] border border-brand-500/30 rounded-xl w-full max-w-md shadow-[0_0_50px_rgba(var(--brand-500),0.1)] overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-white/5 bg-[#12121f]">
              <h3 className="text-lg font-semibold text-white">
                SMPP Credentials
              </h3>
