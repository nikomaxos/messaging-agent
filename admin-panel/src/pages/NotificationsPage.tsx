  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Notifications & Alerts</h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm">Configure alert rules and view triggered notifications.</p>
        </div>
        <button className="bg-brand-600 hover:bg-brand-500 text-slate-900 dark:text-white rounded px-4 py-1.5 text-sm font-medium transition flex items-center gap-2"
      ...
      <div className="glass p-5">
        ...
        {configs.length === 0 ? (
          <div className="text-slate-700 dark:text-slate-500 text-sm text-center py-6">No alert rules configured yet.</div>
        ) : (
          <div className="space-y-2">
            {configs.map((c: any) => (
              <div key={c.id} className={`flex items-center justify-between p-3 rounded-lg border ${c.enabled ? 'bg-white/[0.02] border-slate-300 dark:border-white/5' : 'bg-white dark:bg-slate-900/30 border-slate-800 opacity-60'}`}>
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-700 dark:text-slate-500">
                ...
                <div className="flex items-center gap-2">
                  <button className="text-xs text-slate-600 dark:text-slate-400 hover:text-brand-400 transition" onClick={() => startEdit(c)}>Edit</button>
                  <button className="text-slate-700 dark:text-slate-500 hover:text-red-400 transition" onClick={() => deleteMut.mutate(c.id)}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      <div className="glass p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <AlertTriangle size={14} className="text-amber-600 dark:text-amber-400" /> Active Alerts
          </h2>
        ...
        {activeAlerts.length === 0 ? (
          <div className="text-slate-700 dark:text-slate-500 text-sm text-center py-6">No unacknowledged alerts.</div>
        ) : (
      ...
      <div className="glass p-5 opacity-80">
        <h2 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
          <Info size={14} className="text-slate-600 dark:text-slate-400" /> Archived Alerts (Acknowledged)
        </h2>
        {archivedAlerts.length === 0 ? (
          <div className="text-slate-700 dark:text-slate-500 text-sm text-center py-6">No archived alerts found.</div>
        ) : (
          <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
            {archivedAlerts.map((a: any) => (
              <div key={a.id} className="flex items-center gap-3 px-3 py-2 rounded text-xs bg-slate-100 dark:bg-black/20 border border-slate-300 dark:border-white/5 opacity-70">
                {severityIcon(a.severity)}
      ...
      {/* ── Create/Edit Modal ──────────────────────────────────────── */}
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 dark:bg-black/60 backdrop-blur-sm" onClick={resetForm}>
          <div className="bg-white dark:bg-[#12121f] border border-slate-300 dark:border-white/10 rounded-xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-300 dark:border-white/5 flex items-center justify-between">
              ...
              <button onClick={resetForm} className="text-slate-700 dark:text-slate-500 hover:text-slate-900 dark:text-white transition"><X size={20} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase mb-1">Name</label>
                <input className="w-full bg-white dark:bg-[#0d0d18] text-sm text-slate-900 dark:text-white border border-slate-300 dark:border-white/10 rounded px-3 py-2"
                  value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Low Delivery Warning" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase mb-1">Alert Type</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase mb-1">Threshold</label>
                  <input type="number" className="w-full bg-white dark:bg-[#0d0d18] text-sm text-slate-900 dark:text-white border border-slate-300 dark:border-white/10 rounded px-3 py-2"
                    value={form.threshold} onChange={e => setForm({ ...form, threshold: Number(e.target.value) })} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase mb-1">Cooldown (min)</label>
                  <input type="number" className="w-full bg-white dark:bg-[#0d0d18] text-sm text-slate-900 dark:text-white border border-slate-300 dark:border-white/10 rounded px-3 py-2"
                ...
                {form.type === 'POSSIBLE_AIT_TRAFFIC' && (
                  <div className="col-span-2 mt-2 space-y-3 p-3 bg-red-900/10 border border-red-500/20 rounded-lg">
                    <div className="flex items-start gap-3">
                      <input type="checkbox" checked={form.autoBlock} onChange={e => setForm({ ...form, autoBlock: e.target.checked })} className="mt-1 rounded border-slate-700 bg-white dark:bg-slate-900 text-red-500" />
                      <div>
                        <div className="text-sm font-bold text-red-400">Enable Active Target Auto-Blocking</div>
                        <div className="text-xs text-slate-600 dark:text-slate-400 mt-1">If enabled, any generated alert will actively push the destination phone numbers to the Edge Redis blocklist with a 24-hour TTL, mitigating further SMS pumping attacks.</div>
                      </div>
                    </div>
                    {form.autoBlock && (
                      <div className="pl-7">
                        <label className="block text-[10px] font-bold text-red-400/80 uppercase mb-1">Block Action strategy</label>
                        <select className="w-full bg-slate-100 dark:bg-black/40 text-sm text-red-600 dark:text-red-200 border border-red-500/30 rounded px-3 py-1.5 focus:border-red-500 outline-none"
              ...
              <div className="pt-2">
                <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase mb-2">Notification Channels</label>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                    <input type="checkbox" checked={form.channels.includes('BROWSER_PUSH')} onChange={() => toggleChannel('BROWSER_PUSH')} className="rounded border-slate-700 bg-white dark:bg-slate-900 text-brand-500" />
                    Browser Push
                  </label>
                  <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                    <input type="checkbox" checked={form.channels.includes('RCS_VIRTUAL_SMSC')} onChange={() => toggleChannel('RCS_VIRTUAL_SMSC')} className="rounded border-slate-700 bg-white dark:bg-slate-900 text-brand-500" />
                    RCS via Virtual SMSC
                  </label>
                  {form.channels.includes('RCS_VIRTUAL_SMSC') && (
                    <div className="pl-6 pt-1">
                      <select className="w-full bg-white dark:bg-[#0d0d18] text-sm text-slate-900 dark:text-white border border-slate-300 dark:border-white/10 rounded px-3 py-1.5"
                  ...
                  <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                    <input type="checkbox" checked={form.channels.includes('SMPP_SUPPLIER')} onChange={() => toggleChannel('SMPP_SUPPLIER')} className="rounded border-slate-700 bg-white dark:bg-slate-900 text-brand-500" />
                    SMS via SMPP
                  </label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button className="bg-brand-600 hover:bg-brand-500 text-slate-900 dark:text-white rounded px-4 py-1.5 text-sm font-medium transition disabled:opacity-40"
