...
function LiveUptime({ connectedAt }: { connectedAt?: string }) {
  ...
  useEffect(() => {
    if (!connectedAt) { setVal('έΑΦ'); return }
...
export default function DevicesPage() {
  ...
  const handleRefresh = async () => {
    ...
    setRefreshToast(`έεΥ Refreshed at ${time}`)
  ...
  // έΦΑέΦΑ WebSocket subscription to /topic/devices for live status updates έΦΑέΦΑέΦΑέΦΑέΦΑέΦΑ
  useEffect(() => {
    const token = localStorage.getItem('jwt')
    if (!token) return
  ...
  const sendCommand = async (id: number, command: string) => {
    ...
    if (res.ok && command === 'PIN_AUTOSTART') {
      setAutostartToast('ΏθδκΎ╕Π Autostart protection applied')
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5 flex items-center gap-1.5">
            {wsConnected
              ? <><Wifi size={12} className="text-emerald-400" /> Live έΑΦ updates via WebSocket</>
              : <><WifiOff size={12} className="text-slate-500" /> ReconnectingέΑο</>}
            {refreshToast && (
                ...
                setConfirmAction({
                  ...
                  onConfirm: async () => {
                    ...
                    try {
                      ...
                      setOtaToast(`έεΥ OTA pushed to ${result.sent} device(s)`)
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select className="inp bg-slate-800/70"
                ...
                <option value="">έΑΦ No group έΑΦ</option>
            ...
            {filteredDevices.map((d: Device) => (
              <tr key={d.id}>
                <td className="px-4">
                  ...
                  {d.adbWifiAddress && <div className="text-[10px] text-teal-400 font-mono cursor-pointer hover:text-teal-300 transition" onClick={() => navigator.clipboard.writeText(`adb connect ${d.adbWifiAddress}`)} title="Click to copy adb connect command">ΏθΦΝ {d.adbWifiAddress}</div>}
                ...
                <td className="px-4 text-xs text-slate-300 font-medium">
                  {d.status === 'ONLINE' ? <LiveUptime connectedAt={d.connectedAt} /> : 'έΑΦ'}
                </td>
                <td className="px-4 text-slate-400 text-xs">{d.group?.name ?? 'έΑΦ'}</td>
                <td className="px-4 text-xs">
                  ...
                  ) : <span className="text-slate-300">έΑΦ</span>}
                </td>
                <td className="px-4 text-xs font-semibold text-brand-300">{d.activeNetworkType || 'έΑΦ'}</td>
                <td className={`px-4 text-xs ${d.activeNetworkType === 'WIFI' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}>{d.wifiSignalDbm != null ? `${d.wifiSignalDbm} dBm` : 'έΑΦ'}</td>
                <td className={`px-4 text-xs ${d.activeNetworkType === 'GSM' || d.activeNetworkType === 'CELLULAR' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}>{d.gsmSignalDbm != null ? `${d.gsmSignalDbm} dBm` : 'έΑΦ'}</td>
                ...
                <td className="px-4">
                  {(() => {
                    ...
                    if (!perf) return <span className="text-slate-500 text-xs">έΑΦ</span>
                    const s2h = perf.score2h
                    const s7d = perf.score7d
                    if (!s2h && !s7d) return <span className="text-slate-500 text-xs">έΑΦ</span>
                    ...
                    return (
                      <div className="relative group cursor-help">
                        ...
                        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover:block bg-slate-800 border border-slate-700 text-slate-200 text-[10px] leading-tight rounded-lg px-3 py-2 whitespace-nowrap z-50 shadow-xl">
                          <b>Last 2 Hours</b><br/>
                          έγκ Latency: <b>{s2h?.avgLatencySeconds ?? 0}s</b> avg<br/>
                          έεΖ Delivery: <b>{Math.round((s2h?.successRate ?? 1) * 100)}%</b><br/>
                          ΏθΥρ Dispatched: <b>{s2h?.totalDispatched ?? 0}</b><br/>
                          <br/>
                          <b>Last 7 Days</b><br/>
                          έγκ Latency: <b>{s7d?.avgLatencySeconds ?? 0}s</b> avg<br/>
                          έεΖ Delivery: <b>{Math.round((s7d?.successRate ?? 1) * 100)}%</b><br/>
                          ΏθΥρ Dispatched: <b>{s7d?.totalDispatched ?? 0}</b>
                        </span>
                      </div>
                    )
                  })()}
                      ...
                      <label className="text-[10px] text-slate-500 leading-none mb-0.5 flex items-center gap-0.5">
                        ...
                        <span className="relative group">
                          ...
                          <span className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 hidden group-hover:block bg-slate-800 border border-slate-700 text-slate-200 text-[10px] leading-tight rounded-lg px-3 py-2 whitespace-nowrap z-50 shadow-xl">
                            ...
                            <b>Messages</b> έΑΦ clears Google Messages DB &amp; SMS<br/>
                            <b>System Logs</b> έΑΦ clears Android logcat buffer<br/>
                            <b>All</b> έΑΦ clears both messages and logs
                          </span>
                        </span>
                      </label>
                      <select
                      ...
                      <span className="flex items-center gap-0.5">
                        ...
                        <span className="relative group">
                          ...
                          <span className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 hidden group-hover:block bg-slate-800 border border-slate-700 text-slate-200 text-[10px] leading-tight rounded-lg px-3 py-2 whitespace-nowrap z-50 shadow-xl">
                            ...
                            έΑλ Delivery rate &lt; 50%, OR<br/>
                            έΑλ Avg latency &gt; 30 seconds<br/>
                            Restarts Google Messages bot:<br/>
                            έΑλ After 3 consecutive send-button failures<br/>
