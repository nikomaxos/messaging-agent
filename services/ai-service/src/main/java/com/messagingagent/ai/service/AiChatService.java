package com.messagingagent.ai.service;
...
public class AiChatService {
    ...
    private String getRealModelName(String modelName) {
        if (modelName.contains("gemini-3.1-pro-high")) return "gemini-3.1-pro-preview";
    ...
    private String buildSystemContext() {
        try {
            ...
            Map<String, Object> healthData = restTemplate.getForObject(coreServiceUrl + "/api/system/health", Map.class);
            if (healthData == null) {
                healthData = Map.of("error", "No health data returned");
            }
            
            // Extract some stats for the prompt
            Map<String, Object> pipeline = (Map<String, Object>) healthData.getOrDefault("pipeline", Map.of());
            long queued = pipeline.containsKey("receivedLastHour") && pipeline.containsKey("deliveredLastHour") ? 
                    ((Number)pipeline.get("receivedLastHour")).longValue() - ((Number)pipeline.get("deliveredLastHour")).longValue() : 0;
            
            Map<String, Object> fleet = (Map<String, Object>) healthData.getOrDefault("fleet", Map.of());
            long offline = fleet.containsKey("offline") ? ((Number)fleet.get("offline")).longValue() : 0;
            ...
            return """
                    ...
                    "queuedMessages", Math.max(0, queued),
