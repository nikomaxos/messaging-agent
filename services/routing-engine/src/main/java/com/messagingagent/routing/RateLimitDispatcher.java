...
public class RateLimitDispatcher {
    ...
    private BigDecimal getTpsLimit(String systemId, String country, String network, String supplierId) {
        String key = "config:ratelimit:" + systemId + ":" + country + ":" + network + ":" + supplierId;
        String tpsStr = redis.opsForValue().get(key);
        
        if (tpsStr == null && (!"ALL".equals(country) || !"ALL".equals(network) || !"ALL".equals(supplierId))) {
            String allKey = "config:ratelimit:" + systemId + ":ALL:ALL:ALL";
            tpsStr = redis.opsForValue().get(allKey);
        }
        
        if (tpsStr != null) {
            try {
                return new BigDecimal(tpsStr);
            } catch (NumberFormatException e) {
                log.warn("Invalid TPS value in Redis for key {}: {}", key, tpsStr);
            }
        }
        return BigDecimal.TEN; // Default 10 TPS
    }
}
